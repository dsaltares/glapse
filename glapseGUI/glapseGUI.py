...
class GlapseMainGUI:
	...
	def onBtnScrOutputClicked(self, widget):
		...
		chooser = Gtk.FileChooserDialog(title = title, parent = None, action = action, buttons = buttons)
	...
	# Check folder existence
		if not os.path.exists(output):
			...
			if self.msgQuestion.run() == Gtk.ResponseType.NO:
				checkList = False
			else:
	...
	def checkDependencies(self):
		...
		dependencies = ('ffmpeg','avconv','scrot')

		# This is pretty hacky but if anyone wants to call me out with a better way to support both avconv and ffmpeg feel free
		if not self._which(dependencies[0]) and not self._which(dependencies[0]):
			self.dlgError.set_title(_('Dependencies error'))
			self.dlgError.set_markup(_('<b>ffmpeg or avconv was not found</b>'))
			self.dlgError.format_secondary_text(_('Please, install either package and launch gLapse again'))
			self.dlgError.run()
			self.dlgError.destroy()
			return False
				
		if not self._which(dependencies[0]):
			self.dlgError.set_title(_('Dependencies error'))
			self.dlgError.set_markup(_('<b>%s was not found</b>') % (dependencies[0]))
